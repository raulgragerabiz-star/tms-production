...
/**
 * Rate limiting para /api/integrations/erpclaud/*. Sin esto, un cliente ERP
 * mal configurado (o un token comprometido) podría saturar el bridge de
 * importación sin ninguna contención.
 *
 ...
 *
 * Valores por defecto ajustables por variables de entorno sin tocar código.
 */
...
export const erpclaudCompanyRateLimiter = rateLimit({
  ...
  legacyHeaders: false,
  // Se aplica DESPUÉS de requireAuth en la cadena de middlewares, por eso puede
  // confiar en req.auth.companyId como clave — si por lo que sea no hubiera auth
  // resuelta, cae a la IP para no perder la protección.
