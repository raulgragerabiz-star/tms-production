// Motor de inteligencia (1/3): detección de anomalías.
//
// Adaptado de `_deferred_v1.1_delta/modules/intelligence/anomaly-detection.
// service.ts` (nunca integrado) a la forma real del schema actual -- el
// original asumía `order.orderLines`, aquí la relación se llama `lines`; el
// resto (nombres de tabla/columna en el SQL a medida) ya coincidía tal
// cual. Ver `_manual_merge_needed/schema.delta.prisma` §15 para el
// documento de referencia original.
//
// Principio del documento de origen (16-inteligencia-artificial-TMS.md):
// "vigilancia de patrones fuera de lo habitual... generando alerta para
// revisión humana, no corrección automática". Cada detector de más abajo es
// una función pura que recibe datos ya agregados y decide si hay anomalía
// -- la parte de "traer datos de BD" vive en el orquestador
...
// "un pedido con peso declarado muy distinto al histórico de ese
// cliente/producto". Se compara contra media ± N desviaciones típicas del
// propio cliente -- no contra un umbral global, porque un cliente grande
// (ej. Bigmat) puede pedir habitualmente 10x más que uno pequeño; lo
// relevante es la desviación respecto a SU propio patrón.
export function detectOrderWeightAnomaly(input: WeightAnomalyInput): AnomalyResult {
  ...
  if (input.historicalStdDevWeightKg === 0) {
    ...
    // desviación es potencialmente relevante, pero sin desviación típica no
    // se puede calcular un ratio; se compara contra un 20% de margen fijo
...
// "una liquidación con importe anómalo frente a viajes comparables".
// "Comparables" se resuelve en el orquestador como envíos del mismo
// transportista -- aquí solo se juzga el ratio ya calculado.
...
// "una ruta con ocupación reiteradamente baja". Se evalúa sobre una media
// móvil de N rutas del mismo transportista, NO sobre una ruta puntual --
// una ruta baja de ocupación un día concreto no es anómala por sí sola, un
// patrón sostenido sí lo es (señal para retroalimentar la configuración de
// zonas/consolidación).
...
// "análisis retrospectivo de rutas con desviación relevante entre distancia
// optimizada y distancia real recorrida... para retroalimentar la
// configuración de zonas/consolidación". Mismo patrón de media móvil que
// `detectLowRouteOccupancy`: una ruta puntual con desvío (tráfico puntual,
// desvío por obra) no es señal de nada, un patrón sostenido en el mismo
// transportista sí lo es. Usa `Route.distancePlannedKm`/`distanceRealKm`
// (columnas nuevas, todavía sin alimentar en ningún sitio) -- mientras no
// haya datos, este detector simplemente no genera alertas.
...
async function persistAlertIfNew(params: {
  ...
  try {
    ...
    // ya existe una alerta pendiente idéntica -- comportamiento esperado en
    // ejecuciones repetidas, no es un error real.
...
async function detectAndPersistWeightAnomalies(companyId: string): Promise<number> {
  ...
  // previos) del mismo cliente -- solo pedidos recientes, para no
  // reevaluar indefinidamente historia ya conocida en cada ejecución.
  const recentOrders = await prisma.order.findMany({
    where: { companyId, createdAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } },
    select: { id: true, customerId: true, lines: { select: { lineWeightKg: true } } },
  });

  let count = 0;
  for (const order of recentOrders) {
    const observedWeightKg = order.lines.reduce((sum, l) => sum + Number(l.lineWeightKg ?? 0), 0);
